<template>
  ...
  <router-link to="/communityTSJZ" class="no-style">
    <p class="conductor-viaje">{{ post.nombre_usuario }} {{ post.apellido_usuario }}</p>
  </router-link>
...
methods: {
  ...
  try {
    const response = await fetch('http://localhost:3000/api/viajeEliminarPasajerosSolo', {
      method: 'DELETE',
      headers: {
...
async deletePost(id_viaje){
  try{
    const response = await fetch('http://localhost:3000/api/viajeEliminar',{
      method: 'DELETE',
      headers:{
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('token')}`
...
.posts {
  padding: 20px;
  background-color: #201f2e; /* Morado muy claro */
}
...
.post-item {
  background: linear-gradient(to right, #6e3cc4, #9575cd);
  color: #fff;
  border-radius: 12px;
  padding: 20px;
  margin-bottom: 20px;
...
.progress-fill {
  height: 100%;
  background: linear-gradient(to right, #4d21a5, #aa2424);
...
.fixed-status-button {
  ...
  background-color: #37109F; /* Color de fondo */
